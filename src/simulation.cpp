...
void FEASolver::solve(Mesh& mesh, int num_steps) {
    active_mesh = &mesh;
    std::cout << "  EDGEPREDICT ENGINE v3.4 - FINAL PHYSICS CAP" << std::endl;
...
json FEASolver::solve_time_step(int step_num) {
    ...
    if (cfd_enabled && fluid_solver && particle_system && thermal_model && chip_model) { 
        ...
        double T_melt_limit = 1660.0;
        if (config_data["material_properties"].contains("melting_point_C")) {
            T_melt_limit = config_data["material_properties"]["melting_point_C"].get<double>();
        }
        ...
        // --- NEW: Analytical Pressure Fallback ---
        // If CFD misses the sensor (0 MPa), use analytical Physics Model to estimate pressure
        if (max_rake_pressure < 1e-6) {
             // Approx: Pressure = Cutting Force / Area. 
             // Using a simplified specific cutting energy model for fallback.
             double specific_energy = 3000.0e6; // ~3000 MPa for Ti
...
void FEASolver::update_node_mechanics(Node& node, double dt, bool is_in_contact_zone) {
    ...
    if (is_in_contact_zone) {
        ...
        // Wear is calibrated in physics_models.cpp.
...
void FEASolver::update_node_thermal(Node& node, double dt, double chip_heat_flux, bool is_in_contact_zone) {
    ...
    double T_melt = 1660.0; 
    if (config_data["material_properties"].contains("melting_point_C")) {
        T_melt = config_data["material_properties"]["melting_point_C"].get<double>();
    }
