...
void SimulationEngine::setCFDSolver(CFDSolverGPU* solver) {
    m_cfdSolver = solver;
    if (solver) {
        std::cout << "[Engine] CFD solver registered" << std::endl;
    }
}
...
bool SimulationEngine::step(double dt) {
    ...
    // Step 4.5: Adaptive Optimization (The "Brain")
    // Every 50 steps to avoid excessive PID oscillation
    if (m_optimizationManager && m_strategy && (m_currentStep % 50 == 0)) {
        ...
        // Get actual values from strategy output
        auto output = m_strategy->computeOutput();
        pState.cuttingForce = sqrt(output.cuttingForce.x * output.cuttingForce.x + 
        ...
        m_strategy->applyAdaptiveControl(cmd.feedRateMultiplier, cmd.spindleSpeedMultiplier);
    }
    
    // CFD solver is now part of m_solvers and handled in the loop above
...
void SimulationEngine::loadGeometry() {
    // Phase 1: Skip if solvers already have data (e.g., parametric geometry)
    bool hasData = false;
...
void SimulationEngine::initializeSolvers() {
    ...
    for (auto& solver : m_solvers) {
        // Skip if already initialized or has meshes (e.g. from main.cpp)
        if (solver->getNodeCount() > 0 || solver->getParticleCount() > 0) {
            std::cout << "  - " << solver->getName() << " (already initialized, skipping)" << std::endl;
            continue;
        }
    ...
    if (m_strategy) {
        // Only initialize if not already configured by main (preserves connected solvers)
        if (!m_strategy->isInitialized()) {
             ...
             std::cout << "  - " << m_strategy->getName() << " (already connected, skipping)" << std::endl;
        }
    }
...
void SimulationEngine::exportResults() {
    ...
    // Phase 5.1: High-level Quality Analysis (Orphan Integration)
    // Correctly hook the orphaned roughness and stress predictors
    if (m_strategy) {
        auto output = m_strategy->computeOutput();
        
        if (m_roughnessPredictor) {
            std::cout << "  - [Analytics] Predicting surface roughness..." << std::endl;
            m_roughnessPredictor->predict(output, m_config);
        }
        
        if (m_stressAnalyzer) {
            std::cout << "  - [Analytics] Analyzing residual stress profiles..." << std::endl;
            m_stressAnalyzer->analyze(m_config);
        }
    }
