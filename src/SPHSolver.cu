...
/**
 * @brief Rotate all workpiece particles around Z-axis (GPU kernel)
 * Used by TurningStrategy to avoid slow CPU roundtrip.
 */
__global__ void rotateWorkpieceZKernel(SPHParticle* particles, int numParticles,
                                        double cosA, double sinA, double omega) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= numParticles) return;
    ...
    // Set tangential velocity (not additive — prevents velocity explosion)
    double r = sqrt(newX * newX + newY * newY);
    if (r > 1e-12) {
...
void SPHSolver::rotateWorkpieceZ(double angle, double omega) {
    if (!m_isInitialized || m_numParticles == 0) return;
    ...
    rotateWorkpieceZKernel<<<gridSize, blockSize, 0, m_computeStream>>>(
        d_particles, m_numParticles, cosA, sinA, omega);
    CUDA_CHECK_KERNEL();
}
