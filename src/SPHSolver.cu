 ...
 */
__device__ inline double computeEquivalentStress(double sxx, double syy, double szz,
...
void SPHSolver::initializeParticleBox(const Vec3& minCorner, const Vec3& maxCorner, double spacing) {
    ...
    if (m_mainConfig) {
        ...
        asbConfig.critStrain = m_mainConfig->getJson().value("asb_crit_strain", 0.8);
    }
...
void SPHSolver::updateResults() {
    ...
    // For now, just update from host-side copy
    copyFromDevice();
    
    m_results.maxDensity = 0;
    ...
    m_results.removedParticleCount = 0;
    ...
    for (const auto& p : h_particles) {
        if (p.status == ParticleStatus::ACTIVE) {
            ...
            m_results.totalKineticEnergy += 0.5 * p.mass * (p.vx * p.vx + p.vy * p.vy + p.vz * p.vz);
