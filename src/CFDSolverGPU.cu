...
bool CFDSolverGPU::initialize(const Config& config) {
    ...
    m_params.dt = 0.001f;  // Initial time step
    ...
    std::cout << "[CFDSolverGPU] Memory: " << (m_totalCells * sizeof(float) * 12 / 1024 / 1024) 
              << " MB" << std::endl;
...
void CFDSolverGPU::step(float dt) {
    m_params.dt = dt;
...
void CFDSolverGPU::addForces(float dt) {
    // Gravity and other body forces could be added here
    // For coolant flow, we mainly rely on inlet velocity
}
...
void CFDSolverGPU::updateMetrics() {
    // Find max velocity using reduction
    int n = m_totalCells;
    int threads = 256;
    int blocks = (n + threads - 1) / threads;
...
float CFDSolverGPU::getStableTimeStep() const {
    // CFL condition: dt < dx / max_velocity
    if (m_maxVelocity < 1e-6f) return 0.01f;
    return 0.5f * m_params.dx / m_maxVelocity;
}

void CFDSolverGPU::setSolidObstacles(const float* particlePositions, int numParticles) {
    // Clear solid flags
    CUDA_CHECK(cudaMemset(d_solid, 0, m_totalCells * sizeof(bool)));
    ...
    CUDA_CHECK(cudaMemcpy(d_particlePos, particlePositions, 
...
void CFDSolverGPU::setHeatSources(const float* nodeTemperatures, int numNodes) {
    // Simple: copy temperature as heat source
    // In practice, would interpolate from FEM nodes to grid
    CUDA_CHECK(cudaMemcpy(d_heatSource, nodeTemperatures, 
                          std::min(numNodes, m_totalCells) * sizeof(float), 
                          cudaMemcpyHostToDevice));
}

Vec3 CFDSolverGPU::getVelocityAt(const Vec3& pos) const {
    // Download velocity at position (for coupling)
    // This is slow - in production, would keep fields on GPU
    std::vector<float> u_host(m_totalCells), v_host(m_totalCells), w_host(m_totalCells);
    
    CUDA_CHECK(cudaMemcpy(u_host.data(), d_u, m_totalCells * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(v_host.data(), d_v, m_totalCells * sizeof(float), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(w_host.data(), d_w, m_totalCells * sizeof(float), cudaMemcpyDeviceToHost));
...
float CFDSolverGPU::getTemperatureAt(const Vec3& pos) const {
    std::vector<float> T_host(m_totalCells);
    CUDA_CHECK(cudaMemcpy(T_host.data(), d_T, m_totalCells * sizeof(float), cudaMemcpyDeviceToHost));
...
void CFDSolverGPU::copyVelocityToHost(std::vector<Vec3>& velocities) {
    ...
    for (int i = 0; i < m_totalCells; ++i) {
        velocities[i] = Vec3(u_host[i], v_host[i], w_host[i]);
    }
}

void CFDSolverGPU::copyTemperatureToHost(std::vector<float>& temperatures) {
    temperatures.resize(m_totalCells);
    CUDA_CHECK(cudaMemcpy(temperatures.data(), d_T, m_totalCells * sizeof(float), cudaMemcpyDeviceToHost));
}
