...
// ═══════════════════════════════════════════════════════════
// USUI WEAR MODEL IMPLEMENTATION (CALIBRATED)
// ═══════════════════════════════════════════════════════════
...
double UsuiWearModel::calculate_wear_rate(double temperature_C, double stress_MPa, double sliding_velocity) const {
    ...
    double base_scale = 0.02;
    double base_wear_floor = 1e-15;

    // --- CRITICAL CALIBRATION: Scale raw theoretical wear rate by 1e-7 ---
    // This brings the values into a realistic range for standard machining time steps.
    double calibrated_wear_rate = (base_wear * temp_multiplier * stress_factor * base_scale) * 1e-7;

    return std::max(base_wear_floor, calibrated_wear_rate);
}
...
bool FailureCriterion::check_failure(double stress, double temperature, double accumulated_damage) const {
    if (temperature > 1400.0) return true;
    
    double safe_stress = std::max(stress, 0.0);
    double safe_accumulated_damage = std::max(accumulated_damage, 0.0);
    double temp_K = temperature + 273.15;
    double safe_melting_K = 1668.0 + 273.15;
    double homologous_temp = std::max(0.0, std::min(1.0, temp_K / safe_melting_K));
    ...
    double stress_ratio = safe_stress / effective_uts;
    double base_failure_prob = 1.0 - exp(-pow(stress_ratio, 6));
    double temp_scale = 0.8 + 0.4 * (1.0 - homologous_temp);
    double failure_prob = std::max(0.0, std::min(1.0, base_failure_prob * temp_scale));
...
double FailureCriterion::calculate_damage_increment(double stress, double cycles, double current_damage) const {
    ...
    double damage_increment = base_damage * 0.05 * progressive_factor;
