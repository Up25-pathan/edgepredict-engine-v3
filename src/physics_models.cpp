...
double UsuiWearModel::calculate_wear_rate(double temperature_C, double stress_MPa, double sliding_velocity) const {
    ...
    double base_scale = 0.02; // CALIBRATED: Reduced from 0.1 for a more realistic (longer) tool life.

    // Base wear floor ensures some wear even at low temperatures
    double base_wear_floor = 1e-15; // Minimum wear rate (m/s) - reduced proportionally

    // Ensure final wear rate is non-negative and above floor
    return std::max(base_wear_floor, base_wear * temp_multiplier * stress_factor * base_scale);
}
...
double ThermalModel::calculate_heat_generation(double stress, double strain_rate) const {
    ...
    double base_enhancement = 30.0; // CALIBRATED: Correct value
    ...
    double machining_factor = 1.8; // CALIBRATED: Correct value
...
double ThermalModel::calculate_heat_dissipation(double current_temp, double ambient_temp, double surface_area) const {
    ...
    // --- CALIBRATED: Make cooling much less aggressive ---
    // Minimal cooling coefficient
    double cooling_coeff = 0.25; // CALIBRATED: Correct value
...
bool FailureCriterion::check_failure(double stress, double temperature, double accumulated_damage) const {
    // IMMEDIATE THERMAL FAILURE: Material loses all strength near melting point
    if (temperature > 1400.0) { // CALIBRATED: Lowered from 1550.0 (93% of melt) to ~84%
        return true;
    }
    ...
    // Use actual melting point in Kelvin
    double safe_melting_K = 1668.0 + 273.15; // Ti-6Al-4V melting point
...
double FailureCriterion::calculate_damage_increment(double stress, double cycles, double current_damage) const {
    ...
    double damage_increment = base_damage * 0.05 * progressive_factor; // CALIBRATED: Increased from 0.005
