...
// ═══════════════════════════════════════════════════════════
// THERMAL MODEL IMPLEMENTATION (FIXED UNITS)
// ═══════════════════════════════════════════════════════════
...
double ThermalModel::calculate_heat_generation(double stress, double strain_rate) const {
    ...
    // FIX: Stress comes in as MPa, but Heat Physics needs Pa (N/m^2).
    // We multiply by 1e6 to convert.
    double stress_Pa = safe_stress * 1e6;

    double beta = 0.95; // Taylor-Quinney coefficient (95% of plastic work -> heat)
    double base_heat_flux = beta * stress_Pa * safe_strain_rate;

    // Scaling factor based on relative load
    ...
    // Result is in degrees Celsius per second (K/s)
