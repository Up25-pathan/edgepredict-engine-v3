...
bool TurningStrategy::initialize(const Config& config) {
    ...
    // Parse Initial Position (New safe-start logic)
    std::vector<double> initPos = {0.0, 0.0, 0.0};
    if (config.getJson()["machining_parameters"].contains("initial_tool_position_mm")) {
...
void TurningStrategy::applyKinematics(double dt) {
    ...
    // Update workpiece rotation in SPH solver
    if (m_sph) {
        // Rotate all particles around Z axis (spindle axis)
        double angle = m_conditions.spindleSpeed * dt;
        
        // Get reference to host particles (triggers GPU→CPU copy)
        auto& particles = m_sph->getHostParticles();
        
        double cosA = std::cos(angle);
        double sinA = std::sin(angle);
        
        for (auto& p : particles) {
            if (p.status != ParticleStatus::ACTIVE && p.status != ParticleStatus::CHIP) continue;
            ...
            // SET rotational velocity (not additive — prevents velocity explosion)
            double r = std::sqrt(p.x * p.x + p.y * p.y);
            if (r > 1e-12) {
                double tangentialV = r * m_conditions.spindleSpeed;
                double theta = std::atan2(p.y, p.x);
                // Tangential velocity: perpendicular to radius vector
                p.vx = -tangentialV * std::sin(theta);
                p.vy =  tangentialV * std::cos(theta);
            }
        }
        
        // Write rotated particles back to GPU
        m_sph->updateParticlesFromHost();
    }
...
void TurningStrategy::applyAdaptiveControl(double feedMultiplier, double speedMultiplier) {
    ...
    // Update current conditions immediately
    m_conditions.spindleSpeed *= speedMultiplier;
    m_conditions.feedRate *= feedMultiplier;
}
