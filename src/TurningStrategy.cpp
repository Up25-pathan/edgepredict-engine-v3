...
void TurningStrategy::connectSolvers(SPHSolver* sph, FEMSolver* fem, 
                                      ContactSolver* contact, CFDSolver* cfd) {
    m_sph = sph;
    m_fem = fem;
    m_contact = contact;
    m_cfd = cfd;
...
void TurningStrategy::updateConditions(const MachineState& state, double dt) {
    ...
    m_conditions.spindleSpeed = state.spindleRPM * 2.0 * PI / 60.0;
...
void TurningStrategy::applyKinematics(double dt) {
    ...
    if (m_sph) {
        ...
        for (auto& p : particles) {
            ...
            // Rotate velocity
            double newVx = p.vx * cosA - p.vy * sinA;
            double newVy = p.vx * sinA + p.vy * cosA;
            p.vx = newVx;
            p.vy = newVy;
            ...
            p.vx += -tangentialV * sinA;
            p.vy += tangentialV * cosA;
        }
    }
