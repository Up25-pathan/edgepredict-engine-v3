...
// --- G-Code & Adaptive Control Implementation ---
void TurningStrategy::update_machine_state(const Eigen::Vector3d& pos, double feed, double rpm) {
    if (m_fea_solver) {
...
void TurningStrategy::set_feed_rate(double f) {
    if (m_fea_solver) m_fea_solver->set_feed_rate(f);
}

void TurningStrategy::set_rotation_speed(double s) {
...
void TurningStrategy::initialize(Mesh& mesh, const TopoDS_Shape& cad, const json& config) {
    ...
    // Safe Workpiece Defaults
    std::vector<double> min_def = {0,0,0};
    std::vector<double> max_def = {0.05, 0.05, 0.05};
    json wp = m_config.value("workpiece_setup", json::object());
    
    Eigen::Vector3d min_c(
        wp.value("min_corner", min_def)[0], 
        wp.value("min_corner", min_def)[1], 
        wp.value("min_corner", min_def)[2]
    );
    Eigen::Vector3d max_c(
        wp.value("max_corner", max_def)[0], 
        wp.value("max_corner", max_def)[1], 
        wp.value("max_corner", max_def)[2]
    );
...
json TurningStrategy::run_time_step(double target_dt, int step_num) {
    ...
    const double CHECK_INTERVAL = 0.01; // 10ms check
    ...
    while (time_accumulated < target_dt) {
        ...
        if (m_fluid_solver) m_workpiece_model->apply_fluid_forces(*m_fluid_solver, dt);
