...
SurfaceRoughnessParams SurfaceRoughnessPredictor::predict(const MachiningOutput& output) {
    ...
    // Feed per revolution from chip thickness (rough approximation)
    double feed       = std::max(1e-6, output.chipThickness) * 1000.0;  // m → mm
    double noseRadius = 0.8;  // mm default; ideally from ToolGeometry
    double cuttingSpeed = std::max(1.0, output.chipVelocity * 2.0 * 60.0); // m/s → m/min
    ...
    // BUE effect
    params.Ra_plastic += estimateBUEEffect(cuttingSpeed, output.maxToolTemperature);
...
SurfaceRoughnessParams SurfaceRoughnessPredictor::predictFromParams(
    ...
    if (cuttingSpeed < 50) {                       // BUE common at low speeds
...
double SurfaceRoughnessPredictor::calculateKinematicRoughness(
    ...
    if (noseRadius <= 0) noseRadius = 0.4;
    // Classic formula: Ra = f² / (32·r)  [units: mm, result in μm]
    return (feed * feed) / (32.0 * noseRadius) * 1000.0;
}
