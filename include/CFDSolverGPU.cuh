...
/**
 * @brief GPU-accelerated CFD solver
 */
class CFDSolverGPU {
public:
    CFDSolverGPU();
     ...
     */
    float getTemperatureAt(const Vec3& pos) const;
    
    /**
     * @brief Set solid obstacle from particle positions
     */
    void setSolidObstacles(const float* particlePositions, int numParticles);
    
    /**
     * @brief Set heat sources from FEM temperature
     */
    void setHeatSources(const float* nodeTemperatures, int numNodes);
    ...
    /**
     * @brief Get stable time step
     */
    float getStableTimeStep() const;
    
    /**
     * @brief Copy velocity field to host
     */
    void copyVelocityToHost(std::vector<Vec3>& velocities);
    
    /**
     * @brief Copy temperature field to host
     */
