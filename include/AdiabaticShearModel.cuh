...
/**
 * @brief Configuration for adiabatic shear model
 */
struct AdiabaticShearConfig {
    bool enabled = true;
    
    // Detection criteria
    double criticalStrainRate = 1e4;      // Critical strain rate for ASB (1/s)
    double softeningThreshold = 0.95;     // σ_current / σ_peak ratio for onset
    
    // Thermal properties
    double taylorQuinneyCoeff = 0.9;       // Fraction of plastic work → heat
    double thermalDiffusivity = 2.9e-6;    // m²/s for Ti-6Al-4V
    
    // Band characteristics
    double typicalBandWidth = 20e-6;       // 20 microns typical
    double maxTemperatureRatio = 0.9;      // Max T / T_melt in band
    
    // Segmentation
    double segmentationFrequency = 0;      // Computed shear band spacing (m)
};
